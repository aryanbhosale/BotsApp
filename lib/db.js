const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am 💠Prabal Paul's bitch💠. I guess you were trying to use my commands in``` *{BotsApp.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\n",
        ERROR_TEMPLATE:
        ...
        SUCCESSFUL_CONNECTION: "*Your bitch successfuly integrated.*\n```I am currently working in``` *{worktype}* ```mode.```\n```"
    },
    abl: {
        ...
        CAN_NOT_BLACKLIST_BOT: "```This bitch cannot blacklist itself. Tag or reply to a different user.```",
        GRP_ACKNOWLEDGEMENT: "```{user} has been blacklisted from using the bot in this group.```",
        GRP_BAN: "```Prabal Paul's bitch has been disabled for the group {user}. For more configurations use the``` *.help abl* ```command.```"
    },
    add: {
    ...
    block: {
        ...
        NOT_BLOCK_BOT: "```I cannot block myself```"
    },
    carbon: {
        ...
        OUTPUT: "*<< Here's your carbon image! >>*\n```Carbonized by Prabal Paul's bitch\nColour Scheme: {}```",
    ...
    demote: {
        ...
        BOT_NOT_ADMIN: "```Sorry, dont have the permission to do so since I am not an admin, I'm just a bitch.```",
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION: "```Get the profile picture of the group in a group conversation in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Ye le teri DP, it's so hot that I'm controlling my BP after watching your DP.```",
        PROCESSING: "```Ruk bhai patience...```",
    ...
    help: {
        ...
        HEAD: "🌀 *Bitch Commands* 🌀\n```Here is a list of all the commands my Prabal Paul!```",
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
        COMMAND_INTERFACE: "🌀 *Bot Command Interface* 🌀\n\n",
    ...
    tagall: {
        ...
        TAG_MESSAGE: "*Tumse na ho payega!*"
    },
    ud: {
