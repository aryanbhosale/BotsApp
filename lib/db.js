const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am 💠AryanBot💠. I guess you were trying to use my commands in``` *{BotsApp.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\n",
        ERROR_TEMPLATE:
        ...
        SUCCESSFUL_CONNECTION: "*Bot successfuly integrated.*\n```Bot is currently working in``` *{worktype}* ```mode.```\n```"
    },
    abl: {
    ...
    help: {
        ...
        HEAD: "🌀 *Bot Commands* 🌀\n```Here is a list of all the commands Aryan Sir!```",
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
